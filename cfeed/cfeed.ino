...
#define OpenSens   7
#define ClosedSens 8
#define ValveBTN   9  // Swapped buttons for Italy unit.  8/31/2017 - Billiam
#define ClearBTN   10
...
int UpperSensLastState = 0;
int LowerSensLastState = 0;
int OpenSensLastState = 0;
int ClosedSensLastState = 0;
//}
...
void Closing() {
  ...
  if (digitalRead(ClosedSens) == HIGH) {                // valve not yet closed
...
/*
	CheckSensorState() - A diagnostic function to print sensor state changes.
*/
void CheckSensorState() {
	int x;

	x = digitalRead(UpperSens);
	if (UpperSensLastState != x) {
		Serial.print("Upper sensor state changed to ");
		Serial.println(x);
		UpperSensLastState = x;
	}

	x = digitalRead(LowerSens);
	if (LowerSensLastState != x) {
		Serial.print("Upper sensor state changed to ");
		Serial.println(x);
		LowerSensLastState = x;
	}

	x = digitalRead(OpenSens);
	if (OpenSensLastState != x) {
		Serial.print("Open sensor state changed to ");
		Serial.println(x);
		OpenSensLastState = x;
	}

	x = digitalRead(ClosedSens);
	if (ClosedSensLastState != x) {
		Serial.print("Closed sensor state changed to ");
		Serial.println(x);
		ClosedSensLastState = x;
	}
